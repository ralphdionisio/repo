...
if ( ! function_exists( 'kohaku_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function kohaku_get_setting( $setting ) {
		$kohaku_settings = wp_parse_args(
			get_option( 'kohaku_settings', array() ),
			kohaku_get_defaults()
		);

		return $kohaku_settings[ $setting ];
	}
}

if ( ! function_exists( 'kohaku_get_layout' ) ) {
	/**
	...
	function kohaku_get_layout() {
		...
		$layout = $kohaku_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_kohaku-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $kohaku_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $kohaku_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'kohaku_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'kohaku_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function kohaku_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $kohaku_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_kohaku-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'kohaku_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'kohaku_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function kohaku_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'kohaku_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $kohaku_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'kohaku_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'kohaku_show_title' ) ) {
	/**
	 ...
	 */
	function kohaku_show_title() {
		return apply_filters( 'kohaku_show_title', true );
	}
}

if ( ! function_exists( 'kohaku_padding_css' ) ) {
	/**
	 ...
	 */
	function kohaku_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'kohaku_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'kohaku_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function kohaku_get_navigation_location() {
		return apply_filters( 'kohaku_navigation_location', kohaku_get_setting( 'nav_position_setting' ) );
	}
}
