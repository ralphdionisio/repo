...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Kohaku_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Kohaku_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'kohaku-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'kohaku-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), KOHAKU_VERSION, true );
			wp_enqueue_style( 'kohaku-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), KOHAKU_VERSION );

			wp_enqueue_script( 'kohaku-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'kohaku-typography-selectWoo' ), KOHAKU_VERSION, true );
			wp_enqueue_style( 'kohaku-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), KOHAKU_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'kohaku_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'kohaku' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'kohaku' );
			$this->json[ 'google_fonts' ] = apply_filters( 'kohaku_typography_customize_list', kohaku_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = kohaku_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'kohaku' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'kohaku' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'kohaku' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'kohaku' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="kohaku-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="kohaku-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="kohaku-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="kohaku-font-transform">
					<label>
