( function( api ) {
	...
	api.controlConstructor['kohaku-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.kohaku-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the kohaku_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'kohaku_get_all_google_fonts_ajax',
								kohaku_customize_nonce: kohaku_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.kohaku-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.kohaku-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.kohaku-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.kohaku-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.kohaku-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.kohaku-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.kohaku-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.kohaku-font-variant' );
			...
			control.container.on( 'change', '.kohaku-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.kohaku-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.kohaku-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.kohaku-font-family select' ).selectWoo();
	jQuery( '.kohaku-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".kohaku-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.kohaku-font-variant' ).hide();
		}
	});
