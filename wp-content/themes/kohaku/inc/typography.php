...
if ( ! function_exists( 'kohaku_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'kohaku_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function kohaku_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', kohaku_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $kohaku_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $kohaku_settings[$key], ':' ) !== false ) {
					$kohaku_settings[$key] = current( explode( ':', $kohaku_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $kohaku_settings[$key] );

				// Grab the variants using the plain name
				$variants = kohaku_get_google_font_variants( $kohaku_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'kohaku_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'kohaku_fonts_subset','' );
		...
		$default_fonts = KOHAKU_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('kohaku-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('kohaku-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'kohaku_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'kohaku_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function kohaku_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'kohaku_fonts_customize_register' ) ) {
			// Bail if Kohaku Premium is active
			return;
		}
		...
		$defaults = kohaku_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Kohaku_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'kohaku_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'kohaku_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'kohaku_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Kohaku_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'kohaku' ),
					'settings' => array(
						'family' => 'kohaku_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'kohaku_settings[body_font_weight]',
						'transform' => 'kohaku_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'kohaku_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Kohaku_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'kohaku' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kohaku_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'kohaku_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'kohaku_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Kohaku_Range_Slider_Control(
				...
				array(
					'type' => 'kohaku-range-slider',
					'description' => __( 'Line height', 'kohaku' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kohaku_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'kohaku_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Kohaku_Range_Slider_Control(
				...
				array(
					'type' => 'kohaku-range-slider',
					'description' => __( 'Paragraph margin', 'kohaku' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kohaku_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'kohaku_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function kohaku_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'kohaku_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'kohaku_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'kohaku_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_kohaku_get_all_google_fonts_ajax', 'kohaku_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function kohaku_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'kohaku_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'kohaku_customize_nonce' ] ), 'kohaku_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'kohaku_customize_nonce', 'kohaku_customize_nonce' );
		...
		$fonts = kohaku_get_all_google_fonts();
...
if ( ! function_exists( 'kohaku_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: kohaku_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function kohaku_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, kohaku_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'kohaku_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'kohaku_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: kohaku_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function kohaku_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'kohaku_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function kohaku_get_font_family_css( $font, $settings, $default ) {
		$kohaku_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'kohaku_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $kohaku_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'kohaku_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . kohaku_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'kohaku_add_to_font_customizer_list' ) ) {
	add_filter( 'kohaku_typography_customize_list', 'kohaku_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function kohaku_add_to_font_customizer_list( $fonts ) {
		// Bail if we don't have our defaults
		if ( ! function_exists( 'kohaku_get_default_fonts' ) ) {
			return;
		}
		...
		$all_fonts = kohaku_get_all_google_fonts();
		$select_fonts = kohaku_get_all_google_fonts( apply_filters( 'kohaku_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $kohaku_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $kohaku_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $kohaku_settings[ $setting ], kohaku_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $kohaku_settings[ $setting ] ) ) ] = array(
				'name' => $kohaku_settings[ $setting ],
		...
		if ( function_exists( 'kohaku_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'kohaku_secondary_nav_settings', array() ),
				kohaku_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], kohaku_typography_default_fonts() ) ) {
