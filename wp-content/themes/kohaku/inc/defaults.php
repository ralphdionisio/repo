<?php
/**
 * Sets all of our theme defaults.
 *
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

if ( ! function_exists( 'kohaku_get_defaults' ) ) {
	/**
	 * Set default options
	 *
	 */
	function kohaku_get_defaults() {
		$kohaku_defaults = array(
			'hide_title' => '',
			'hide_tagline' => true,
			'top_bar_width' => 'full',
			'top_bar_inner_width' => 'full',
			'top_bar_alignment' => 'left',
			'container_width' => '2000',
			'header_layout_setting' => 'fluid-header',
			'header_inner_width' => 'full-width',
			'nav_alignment_setting' => 'right',
			'header_alignment_setting' => 'left',
			'nav_layout_setting' => 'contained-nav',
			'nav_inner_width' => 'contained',
			'nav_position_setting' => 'nav-float-left',
			'nav_dropdown_type' => 'hover',
			'nav_search' => 'enable',
			'content_layout_setting' => 'separate-containers',
			'layout_setting' => 'no-sidebar',
			'blog_layout_setting' => 'right-sidebar',
			'single_layout_setting' => 'right-sidebar',
			'fixed_side_content' => '',
			'blog_header_image' => '',
			'blog_header_image_2' => '',
			'blog_header_title' => '',
			'blog_header_text' => '',
			'blog_header_button_text' => '',
			'blog_header_button_url' => '',
			'post_content' => 'excerpt',
			'footer_layout_setting' => 'fluid-footer',
			'footer_widgets_inner_width' => 'contained',
			'footer_inner_width' => 'full-width',
			'footer_widget_setting' => '4',
			'footer_copyright' => __( '&copy; 2018 All rights reserved.', 'kohaku' ),
			'footer_bar_alignment' => 'right',
			'back_to_top' => 'enable',
			'side_inside_color' => '#ffffff',
			'text_color' => '#555555',
			'link_color' => '#222222',
			'link_color_hover' => '#555555',
			'link_color_visited' => '',
			'font_awesome_essentials' => true,
		);

		return apply_filters( 'kohaku_option_defaults', $kohaku_defaults );
	}
}

if ( ! function_exists( 'kohaku_get_color_defaults' ) ) {
	/**
	 * Set default options
	 */
	function kohaku_get_color_defaults() {
		$kohaku_color_defaults = array(
			'top_bar_background_color' => '#374855',
			'top_bar_text_color' => '#ffffff',
			'top_bar_link_color' => '#ffffff',
			'top_bar_link_color_hover' => '#dddddd',
			'header_background_color' => '',
			'header_text_color' => '',
			'header_link_color' => '',
			'header_link_hover_color' => '',
			'site_title_color' => '',
			'site_tagline_color' => '',
			'navigation_background_color' => '#ffffff',
			'navigation_text_color' => '#222222',
			'navigation_background_hover_color' => '',
			'navigation_text_hover_color' => '#555555',
			'navigation_background_current_color' => '',
			'navigation_text_current_color' => '',
			'subnavigation_background_color' => '#222222',
			'subnavigation_text_color' => '#ffffff',
			'subnavigation_background_hover_color' => '',
			'subnavigation_text_hover_color' => '#cccccc',
			'subnavigation_background_current_color' => '',
			'subnavigation_text_current_color' => '',
			'fixed_side_content_background_color' => '',
			'fixed_side_content_text_color' => '#ffffff',
			'fixed_side_content_link_color' => '#dddddd',
			'fixed_side_content_link_hover_color' => '#ffffff',
			'content_background_color' => '',
			'content_text_color' => '',
			'content_link_color' => '',
			'content_link_hover_color' => '',
			'content_title_color' => '',
			'blog_post_title_color' => '',
			'blog_post_title_hover_color' => '',
			'entry_meta_text_color' => '',
			'entry_meta_link_color' => '',
			'entry_meta_link_color_hover' => '',
			'h1_color' => '#222222',
			'h2_color' => '#222222',
			'h3_color' => '#222222',
			'h4_color' => '',
			'h5_color' => '',
			'h6_color' => '',
			'sidebar_widget_background_color' => '',
			'sidebar_widget_text_color' => '',
			'sidebar_widget_link_color' => '',
			'sidebar_widget_link_hover_color' => '',
			'sidebar_widget_title_color' => '',
			'footer_widget_background_color' => '#374855',
			'footer_widget_text_color' => '#ffffff',
			'footer_widget_link_color' => '#dddddd',
			'footer_widget_link_hover_color' => '#ffffff',
			'footer_widget_title_color' => '#ffffff',
			'footer_background_color' => '#ffffff',
			'footer_text_color' => '#222222',
			'footer_link_color' => '#222222',
			'footer_link_hover_color' => '#555555',
			'form_background_color' => '#fafafa',
			'form_text_color' => '#555555',
			'form_background_color_focus' => '#ffffff',
			'form_text_color_focus' => '#555555',
			'form_border_color' => '#cccccc',
			'form_border_color_focus' => '#bfbfbf',
			'form_button_background_color' => '#eeeeee',
			'form_button_background_color_hover' => '#374855',
			'form_button_text_color' => '#222222',
			'form_button_text_color_hover' => '#ffffff',
			'back_to_top_background_color' => 'rgba(255,255,255,0.7)',
			'back_to_top_background_color_hover' => '#ffffff',
			'back_to_top_text_color' => '#374855',
			'back_to_top_text_color_hover' => '#374855',
		);

		return apply_filters( 'kohaku_color_option_defaults', $kohaku_color_defaults );
	}
}

if ( ! function_exists( 'kohaku_get_default_fonts' ) ) {
	/**
	 * Set default options.
	 *
	 *
	 * @param bool $filter Whether to return the filtered values or original values.
	 * @return array Option defaults.
	 */
	function kohaku_get_default_fonts( $filter = true ) {
		$kohaku_font_defaults = array(
			'font_body' => 'Roboto',
			'font_body_category' => '',
			'font_body_variants' => '100,100italic,300,300italic,regular,italic,500,500italic,700,700italic,900,900italic',
			'body_font_weight' => 'normal',
			'body_font_transform' => 'none',
			'body_font_size' => '19',
			'body_line_height' => '1.5', // no unit
			'paragraph_margin' => '1.5', // em
			'font_top_bar' => 'Roboto Condensed',
			'font_top_bar_category' => '',
			'font_top_bar_variants' => '300,300italic,regular,italic,700,700italic',
			'top_bar_font_weight' => 'normal',
			'top_bar_font_transform' => 'none',
			'top_bar_font_size' => '19',
			'font_site_title' => 'Roboto Slab',
			'font_site_title_category' => '',
			'font_site_title_variants' => '100,300,regular,700',
			'site_title_font_weight' => '700',
			'site_title_font_transform' => 'none',
			'site_title_font_size' => '50',
			'mobile_site_title_font_size' => '25',
			'font_site_tagline' => 'inherit',
			'font_site_tagline_category' => '',
			'font_site_tagline_variants' => '',
			'site_tagline_font_weight' => 'normal',
			'site_tagline_font_transform' => 'none',
			'site_tagline_font_size' => '15',
			'font_navigation' => 'Roboto Condensed',
			'font_navigation_category' => '',
			'font_navigation_variants' => '300,300italic,regular,italic,700,700italic',
			'navigation_font_weight' => 'normal',
			'navigation_font_transform' => 'uppercase',
			'navigation_font_size' => '20',
			'font_widget_title' => 'Roboto Slab',
			'font_widget_title_category' => '',
			'font_widget_title_variants' => '100,300,regular,700',
			'widget_title_font_weight' => 'normal',
			'widget_title_font_transform' => 'none',
			'widget_title_font_size' => '25',
			'widget_title_separator' => '18',
			'widget_content_font_size' => '19',
			'font_buttons' => 'Roboto Slab',
			'font_buttons_category' => '',
			'font_buttons_variants' => '100,300,regular,700',
			'buttons_font_weight' => 'normal',
			'buttons_font_transform' => 'none',
			'buttons_font_size' => '19',
			'font_heading_1' => 'Roboto Slab',
			'font_heading_1_category' => '',
			'font_heading_1_variants' => '100,300,regular,700',
			'heading_1_weight' => 'normal',
			'heading_1_transform' => 'none',
			'heading_1_font_size' => '80',
			'heading_1_line_height' => '1.2', // em
			'mobile_heading_1_font_size' => '30',
			'font_heading_2' => 'Roboto Slab',
			'font_heading_2_category' => '',
			'font_heading_2_variants' => '100,300,regular,700',
			'heading_2_weight' => 'normal',
			'heading_2_transform' => 'none',
			'heading_2_font_size' => '42',
			'heading_2_line_height' => '1.2', // em
			'mobile_heading_2_font_size' => '25',
			'font_heading_3' => 'Roboto Slab',
			'font_heading_3_category' => '',
			'font_heading_3_variants' => '100,300,regular,700',
			'heading_3_weight' => 'normal',
			'heading_3_transform' => 'none',
			'heading_3_font_size' => '25',
			'heading_3_line_height' => '1.2', // em
			'font_heading_4' => 'inherit',
			'font_heading_4_category' => '',
			'font_heading_4_variants' => '',
			'heading_4_weight' => 'normal',
			'heading_4_transform' => 'none',
			'heading_4_font_size' => '',
			'heading_4_line_height' => '', // em
			'font_heading_5' => 'inherit',
			'font_heading_5_category' => '',
			'font_heading_5_variants' => '',
			'heading_5_weight' => 'normal',
			'heading_5_transform' => 'none',
			'heading_5_font_size' => '',
			'heading_5_line_height' => '', // em
			'font_heading_6' => 'inherit',
			'font_heading_6_category' => '',
			'font_heading_6_variants' => '',
			'heading_6_weight' => 'normal',
			'heading_6_transform' => 'none',
			'heading_6_font_size' => '',
			'heading_6_line_height' => '', // em
			'font_footer' => 'Roboto Condensed',
			'font_footer_category' => '',
			'font_footer_variants' => '300,300italic,regular,italic,700,700italic',
			'footer_weight' => 'normal',
			'footer_transform' => 'none',
			'footer_font_size' => '20',
			'font_fixed_side' => 'Roboto Condensed',
			'font_fixed_side_category' => '',
			'font_fixed_side_variants' => '300,300italic,regular,italic,700,700italic',
			'fixed_side_font_weight' => 'normal',
			'fixed_side_font_transform' => 'none',
			'fixed_side_font_size' => '28',
		);

		if ( $filter ) {
			return apply_filters( 'kohaku_font_option_defaults', $kohaku_font_defaults );
		}

		return $kohaku_font_defaults;
	}
}

if ( ! function_exists( 'kohaku_spacing_get_defaults' ) ) {
	/**
	 * Set the default options.
	 *
	 *
	 * @param bool $filter Whether to return the filtered values or original values.
	 * @return array Option defaults.
	 */
	function kohaku_spacing_get_defaults( $filter = true ) {
		$kohaku_spacing_defaults = array(
			'top_bar_top' => '8',
			'top_bar_right' => '0',
			'top_bar_bottom' => '5',
			'top_bar_left' => '0',
			'header_top' => '25',
			'header_right' => '30',
			'header_bottom' => '10',
			'header_left' => '30',
			'fixed_side_top' => '0',
			'fixed_side_right' => '0',
			'fixed_side_bottom' => '8',
			'fixed_side_left' => '100',
			'menu_item' => '5',
			'menu_item_height' => '65',
			'sub_menu_item_height' => '10',
			'content_top' => '15',
			'content_right' => '30',
			'content_bottom' => '15',
			'content_left' => '30',
			'mobile_content_top' => '15',
			'mobile_content_right' => '15',
			'mobile_content_bottom' => '15',
			'mobile_content_left' => '15',
			'side_top' => '0',
			'side_right' => '0',
			'side_bottom' => '0',
			'side_left' => '60',
			'mobile_side_top' => '0',
			'mobile_side_right' => '0',
			'mobile_side_bottom' => '0',
			'mobile_side_left' => '30',
			'separator' => '15',
			'left_sidebar_width' => '25',
			'right_sidebar_width' => '25',
			'widget_top' => '0',
			'widget_right' => '30',
			'widget_bottom' => '10',
			'widget_left' => '10',
			'footer_widget_container_top' => '60',
			'footer_widget_container_right' => '30',
			'footer_widget_container_bottom' => '40',
			'footer_widget_container_left' => '30',
			'footer_widget_separator' => '30',
			'footer_top' => '8',
			'footer_right' => '0',
			'footer_bottom' => '5',
			'footer_left' => '0',
		);

		if ( $filter ) {
			return apply_filters( 'kohaku_spacing_option_defaults', $kohaku_spacing_defaults );
		}

		return $kohaku_spacing_defaults;
	}
}

if ( ! function_exists( 'kohaku_get_default_color_palettes' ) ) {
	/**
	 * Set up our colors for the color picker palettes and filter them so you can change them.
	 *
	 */
	function kohaku_get_default_color_palettes() {
		$palettes = array(
			'#374855',
			'#ffffff',
			'#dddddd',
			'#555555',
			'#333333',
			'#222222'
		);

		return apply_filters( 'kohaku_default_color_palettes', $palettes );
	}
}

if ( ! function_exists( 'kohaku_typography_default_fonts' ) ) {
	/**
	 * Set the default system fonts.
	 *
	 */
	function kohaku_typography_default_fonts() {
		$fonts = array(
			'inherit',
			'System Stack',
			'Arial, Helvetica, sans-serif',
			'Courier New',
			'Georgia, Times New Roman, Times, serif',
			'Trebuchet MS, Helvetica, sans-serif',
			'Verdana, Geneva, sans-serif',
			'Roboto',
			'Roboto Condensed',
			'Roboto Slab'
		);

		return apply_filters( 'kohaku_typography_default_fonts', $fonts );
	}
}

define('KOHAKU_DEFAULT_FONTS','//fonts.googleapis.com/css?family=Roboto+Slab:100,300,regular,700|Roboto+Condensed:300,300italic,regular,italic,700,700italic|Roboto:100,100italic,300,300italic,regular,italic,500,500italic,700,700italic,900,900italic');
