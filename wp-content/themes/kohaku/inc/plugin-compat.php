...
add_action( 'after_setup_theme', 'kohaku_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function kohaku_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'kohaku_construct_sidebars' );
}

if ( ! function_exists( 'kohaku_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'kohaku_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function kohaku_woocommerce_start() { ?>
...
if ( ! function_exists( 'kohaku_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'kohaku_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function kohaku_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'kohaku_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kohaku_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function kohaku_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'kohaku_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'kohaku_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kohaku_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function kohaku_bbpress_css() {
...
if ( ! function_exists( 'kohaku_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kohaku_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function kohaku_buddypress_css() {
...
if ( ! function_exists( 'kohaku_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kohaku_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function kohaku_beaver_builder_css() {
